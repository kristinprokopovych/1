        ...
        stage('Білд Docker зображення') {
            steps {
                script {
                    ...
                    sh 'docker build -t kristine2208/jen:version${BUILD_NUMBER} .'
                }
            }
        }
          ...
          stage('Тегування Docker зображення') {
            steps {
                script {
                    ...
                    sh 'docker tag kristine2208/jen:version${BUILD_NUMBER} kristine2208/jen:latest'
                }
            }
        }
        ...
        stage('Пуш у Docker Hub') {
            steps {
                script {
                    ...
                    sh 'docker push kristine2208/jen:version${BUILD_NUMBER}'
                    sh 'docker push kristine2208/jen:latest'
                }
            }
        }
        ...
        stage('Запуск Docker контейнера') {
            steps {
                script {
                    ...
                    sh 'docker run -d -p 8081:80 --name ${CONTAINER_NAME} --health-cmd="curl --fail http://localhost:80 || exit 1"  kristine2208/jen:version${BUILD_NUMBER}'
